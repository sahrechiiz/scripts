...
echo -ne "Download TCP LocoX Source code\n"
[ -e ./tmp ] && rm -rf ./tmp
mkdir -p ./tmp && cd ./tmp
[ $? -eq '0' ] && {
wget --no-check-certificate -qO- 'https://raw.githubusercontent.com/sahrechiiz/TCP_BBR_MOD/master/tcp_locox.c' >./tcp_locox.c
echo 'obj-m:=tcp_locox.o' >./Makefile
make -s -C /lib/modules/$(uname -r)/build M=`pwd` modules CC=`which gcc`
echo "Loading TCP LOCOX..."
[ -f ./tcp_locox.ko ] && [ -f /lib/modules/$(uname -r)/modules.dep ] && {
cp -rf ./tcp_locox.ko /lib/modules/$(uname -r)/kernel/net/ipv4
...
echo "net.ipv4.tcp_congestion_control = tcp_locox" >>/etc/sysctl.conf
}
lsmod |grep -q 'tcp_locox'
[ $? -eq '0' ] && {
sysctl -p >/dev/null 2>&1
echo "Finish! "
exit 0
} || {
echo "Error, Loading TCP LOCOX."
exit 1
}
